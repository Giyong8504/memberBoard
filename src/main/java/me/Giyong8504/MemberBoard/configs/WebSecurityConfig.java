...
//                .sessionCreationPolicy(SessionCreationPolicy.STATELESS);
        ...
        http.authorizeHttpRequests(c -> {
            c.requestMatchers("/admin/**").hasAuthority("ADMIN") // "/admin/" 경로 요청은 'ADMIN' 권한을 가진 사용자만 접근 가능
                    //.requestMatchers("/myPage/**", "/new-board", "/changePassword/**", "/deleteId/**").hasAnyAuthority("USER", "GOOGLE","ADMIN") // OAuth 로그인 사용자는 접근이 안되므로 보류.
                    .requestMatchers("/myPage/**", "/new-board", "/changePassword/**", "/deleteId/**").authenticated() // 해당 경로 요청은 인증된 사용자만 접근 가능
